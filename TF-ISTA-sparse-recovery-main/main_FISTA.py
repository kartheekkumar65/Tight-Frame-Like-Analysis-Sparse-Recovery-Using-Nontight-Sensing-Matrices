...
parser = ArgumentParser(description='FISTA')
...
iter_dict = {0.001: 10000, 0.005: 5000, 0.003: 5000, 0.01: 5000, 0.03: 3000, 0.05: 1000, 0.1:1000, 0:10000}
...
net = FISTA(m, n, D, numLayers, alpha = alpha, device = device)
net.weights_init()
...
print('Testing: my FISTA avg SNR is ', np.mean(SNR_list))
print('Testing: my FISTA std SNR is ', np.std(SNR_list))
print('Testing: my FISTA avg PES is ', PES_mean)
print('Testing: my FISTA std PES is ', PES_std)
...
sio.savemat(f'stored_results/FISTA_iter_{numIter}_noise_{noise_level:.3f}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations', RSNR_dict)

output_file_name = f"./log/FISTA_iter_{numIter}_noise_{noise_level}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations.txt"
