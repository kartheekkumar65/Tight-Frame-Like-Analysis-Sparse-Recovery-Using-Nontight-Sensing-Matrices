...
parser = ArgumentParser(description='Rescaled-Tight-Frame-FISTA')
...
class ISTA(nn.Module):
    ...
    def weights_init(self):
        ...
        AAt_inv = LA.inv(AAt)
        At_AAt_inv = np.matmul(A.T, AAt_inv)
        
        # normalize the rows of At_AAt_inv to make diagonal entries 
        # of At_AAt_inv_A equal to 1. That makes it an UNTF
        At_AAt_inv_A = np.matmul(At_AAt_inv, A)
        C = np.diag(np.diag(At_AAt_inv_A)); C = LA.inv(C)
        C_At_AAt_inv = np.matmul(C, At_AAt_inv)
        C_At_AAt_inv_A = np.matmul(C_At_AAt_inv, A)
        alpha = (np.linalg.norm(C_At_AAt_inv_A, 2) )*1.001
        
        S = torch.from_numpy(np.eye(A.shape[1]) - (1/alpha)*np.matmul(C_At_AAt_inv, A))
        
        S = S.float().to(self.device)
        B = torch.from_numpy((1/alpha)*C_At_AAt_inv)
    ...
    def forward(self, y):
        ...
        for iter in range(self.numIter):
            ...
            d = soft_thr(torch.mm(self.S, z.T).T + torch.mm(self.W, y.T).T, self.thr)
            x.append(d)
...
print('Testing: my RTF-FISTA avg SNR is ', np.mean(SNR_list))
print('Testing: my RTF-FISTA std SNR is ', np.std(SNR_list))
print('Testing: my RTF-FISTA avg PES is ', PES_mean)
print('Testing: my RTF-FISTA std PES is ', PES_std)
...
sio.savemat(f'stored_results/RTF_FISTA_iter_{numIter}_noise_{noise_level:.3f}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations', RSNR_dict)

output_file_name = f"./log/RTF_FISTA_iter_{numIter}_noise_{noise_level}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations.txt"
