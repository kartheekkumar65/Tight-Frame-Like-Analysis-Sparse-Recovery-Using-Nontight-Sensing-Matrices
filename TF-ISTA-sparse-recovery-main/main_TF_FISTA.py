...
parser = ArgumentParser(description='Tight-Frame-FISTA')
...
print('Testing: my TF-FISTA avg SNR is ', np.mean(SNR_list))
print('Testing: my TF-FISTA std SNR is ', np.std(SNR_list))
print('Testing: my TF-FISTA avg PES is ', PES_mean)
print('Testing: my TF-FISTA std PES is ', PES_std)
...
sio.savemat(f'stored_results/TF_FISTA_iter_{numIter}_noise_{noise_level:.3f}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations', RSNR_dict)

output_file_name = f"./log/TF_FISTA_iter_{numIter}_noise_{noise_level}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations.txt"
