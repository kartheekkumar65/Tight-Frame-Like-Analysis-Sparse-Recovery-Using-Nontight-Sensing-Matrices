...
parser = ArgumentParser(description='Tight-Frame-ISTA')
...
class ISTA(nn.Module):
    ...
    def weights_init(self):
        ...
        # AAt_inv_half = LA.sqrtm(AAt_inv)
        # A = np.matmul(AAt_inv_half, A)
        S = torch.from_numpy(np.eye(A.shape[1]) - (1/alpha)*np.matmul(At_AAt_inv, A))
        
        S = S.float().to(self.device)
        B = torch.from_numpy((1/alpha)*At_AAt_inv)
...
print('Testing: my TF-ISTA avg SNR is ', np.mean(SNR_list))
print('Testing: my TF-ISTA std SNR is ', np.std(SNR_list))
print('Testing: my TF-ISTA avg PES is ', PES_mean)
print('Testing: my TF-ISTA std PES is ', PES_std)

#%% plot objective function of ISTA
...
sio.savemat(f'stored_results/TF_ISTA_iter_{numIter}_noise_{noise_level:.3f}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations', RSNR_dict)

output_file_name = f"./log/TF_ISTA_iter_{numIter}_noise_{noise_level}_sparsity_{sparsity}_sensing_{sensing}_vs_iterations.txt"
...
# plt.figure()
# markerline1, stemlines, baseline = plt.stem(range(100),X_test[:,25],markerfmt ='D',label='original')
# markerline, stemlines, baseline = plt.stem(range(100),X_out[:,25],markerfmt ='*',label='ISTA')
# markerline1.set_markerfacecolor('none')
# plt.xlabel('Sparse coefficient index')
# plt.ylabel('Amplitude')
# # plt.title('Estimated vs Original noise level {}, sparsity {}'.format(input_SNR, sparsity))
# plt.legend()
# # plt.savefig(f'figures/f_{t[1]}-{t[2]}-{t[3]}-{t[4]}.png')
# plt.show()
